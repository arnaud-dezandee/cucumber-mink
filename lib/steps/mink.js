...
function setBaseUrl (url, callback) {
  this.browser.site = parseUrl(url);
  callback();
}
...
function visit (url, callback) {
  this.browser.visit(parseUrl(url), function (error) {
    if (error) {
...
function assertPageAddress (url, callback) {
  expect(this.browser.location.pathname).to.equal(parseUrl(url));
  callback();
}
...
function assertUrlParamRegExp (pattern, callback) {
  expect(this.browser.location.search).to.match(new RegExp(pattern));
  callback();
}
...
function assertCheckboxChecked (checkbox, callback) {
  ...
  if (checkboxNode) {
    expect(checkboxNode.checked).to.be.equal(true);
    callback();
  } else {
...
function assertCheckboxNotChecked (checkbox, callback) {
  var checkboxNode = this.browser.field(checkbox);
  if (checkboxNode) {
    expect(checkboxNode.checked).to.be.equal(false);
    callback();
  } else {
    callback.fail(new Error('Unable to find checkbox field with selector|name : ' + checkbox));
  }
...
module.exports = {
  ...
  assertUrlParamRegExp: assertUrlParamRegExp,
